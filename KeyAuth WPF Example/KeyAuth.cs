    ...
    public class api
    {
        ...
        public api(string name, string ownerid, string secret, string version, string? path = null)
        {
            if (ownerid.Length != 10 || secret.Length != 64)
            {
                ...
                Process.Start(new ProcessStartInfo("https://keyauth.cc/app/") { UseShellExecute = true });
                Thread.Sleep(2000);
        ...
        private static async Task<string> req(NameValueCollection post_data)
        {
            try
            {
                ...
                {
                    handler.Proxy = null;
                    handler.ServerCertificateCustomValidationCallback = (message, cert, chain, errors) => assertSSL(message, cert, chain, errors);

                    using (HttpClient client = new HttpClient(handler))
                    {
                        using (var content = new FormUrlEncodedContent(ConvertToDictionary(post_data)))
                        {
                            Stopwatch stopwatch = new Stopwatch();
                            stopwatch.Start();

                            HttpResponseMessage response = await client.PostAsync("https://keyauth.win/api/1.2/", content);
                            ...
                            var raw_response = await response.Content.ReadAsStringAsync();
                            var signature = response.Headers.Contains("signature") ? response.Headers.GetValues("signature").FirstOrDefault() : null;

                            sigCheck(raw_response, signature, post_data.Get(0));

                            LogEvent(raw_response + "\n");
            ...
            {
                // Handle errors specific to HTTP requests
                if (httpEx.StatusCode == (HttpStatusCode)429)
                {
        ...
        private static Dictionary<string, string> ConvertToDictionary(NameValueCollection nvc)
        {
            return nvc.Cast<string>().ToDictionary(key => key, key => nvc[key]);
        }
